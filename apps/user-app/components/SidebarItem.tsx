...
export const SidebarItem = ({ href, title, icon }: { href: string; title: string; icon: React.ReactNode }) => {
    ...
    return (
        <div onClick={() => router.push(href)}
            className={`group relative flex items-center gap-3 px-8 py-3 cursor-pointer transition-all duration-200 ease-in-out
                ${selected 
            ...
            {/* Hover effect line */}
            <div className={`
                absolute left-0 top-0 h-full w-1 bg-gradient-to-b from-emerald-600 to-teal-800 transform transition-all duration-200
            ...
            {/* Title */}
            <div className={`
                font-medium hidden md:block transition-colors duration-200
                ${selected 
